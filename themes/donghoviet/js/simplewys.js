...
//Thêm ảnh từ Gallery
function addGallery(gallery_filename, id){
	
	var gal_bbcode = "[gallery]" + gallery_filename + "[/gallery]";
	...
	if (isFF == true){
		...
		myArea.value = begin + gal_bbcode + end;
		//gan lai con tro
		myArea.setSelectionRange(start_selection + gal_bbcode.length,start_selection + gal_bbcode.length);
		myArea.scrollTop = scrollpos;
		...
		//Lay vi tri con tro
		cursor = document.selection.createRange();
		cursor.text = gal_bbcode + cursor.text;	
	}
	//Các loại trình duyệt khác
	else{
			myArea.value+=gal_bbcode;
			myArea.focus();
	}
}
...
function addArticleCode(t,id){
	...
	if(textSelected == true){
		...
		//IE
			...
			var IE_range = myArea.createTextRange();
			//Doi voi IE remove het \n roi moi moveStart den, neu ko se ko chinh xac
			var IE_begin_selection = begin.replace(/\n/g, "").length;
			IE_range.collapse(true);
			IE_range.moveEnd('character', IE_begin_selection);
			IE_range.moveStart('character', IE_begin_selection);
			IE_range.select();
		}
	...
	if(textSelected == false){
		alert("Bạn hãy chọn chuỗi text cần định dạng !");
	}
}
