...
if ($pro_config['home_view'] != "view_home_cat") {
	...
	$sql = "SELECT SQL_CALC_FOUND_ROWS id,listcatid, publtime, " . NV_LANG_DATA . "_title," . NV_LANG_DATA . "_alias, " . NV_LANG_DATA . "_hometext, " . NV_LANG_DATA . "_address,  homeimgalt, homeimgthumb, product_price,product_discounts, money_unit,showprice, homeimgfile FROM `" . $db_config['prefix'] . "_" . $module_data . "_rows` WHERE inhome=1 AND status=1 AND publtime <= " . NV_CURRENTTIME . " AND (exptime=0 OR exptime>" . NV_CURRENTTIME . ") " . $order_by . " LIMIT " . $page . "," . $per_page . "";
	...
	foreach ($global_array_cat as $catid_i => $array_info_i) {
		if ($array_info_i['parentid'] == 0 and $array_info_i['inhome'] == 1) {$array_cat = array();
			$array_cat = GetCatidInParent($catid_i);
			$s = "SELECT SQL_CALC_FOUND_ROWS id, publtime, " . NV_LANG_DATA . "_title," . NV_LANG_DATA . "_alias, " . NV_LANG_DATA . "_hometext, " . NV_LANG_DATA . "_address, homeimgalt, homeimgthumb, product_price,product_discounts, money_unit,showprice,homeimgfile  FROM `" . $db_config['prefix'] . "_" . $module_data . "_rows` WHERE listcatid IN (" . implode(",", $array_cat) . ") AND inhome=1 AND status=1 AND publtime < " . NV_CURRENTTIME . " AND (exptime=0 OR exptime>" . NV_CURRENTTIME . ") ORDER BY ID DESC LIMIT 0," . $array_info_i['numlinks'] . "";
			$re = $db -> sql_query($s);
			$result = $db -> sql_query("SELECT FOUND_ROWS()");
			list($num_pro) = $db -> sql_fetchrow($result);
			$link = NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=";
			$data_pro = array();
			while (list($id, $publtime, $title, $alias, $hometext, $address, $homeimgalt, $homeimgthumb, $product_price, $product_discounts, $money_unit, $showprice, $homeimgfile) = $db -> sql_fetchrow($re)) {
			$thumb = explode( "|", $homeimgthumb );
				$homeimgfile = NV_BASE_SITEURL . NV_UPLOADS_DIR . "/" . $module_name . "/".$homeimgfile;
                if ( ! empty( $thumb[0] ) && ! nv_is_url( $thumb[0] ) )
