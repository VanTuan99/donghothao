...
if ($ac == 0) {
	if ($id > 0) {
		...
		if (! isset ( $_SESSION ['cart'] [$id] ))
			$_SESSION ['cart'] [$id] = array ('num' => $num, 'order' => 0 ,'price' => $price_product_discounts);
		else {
			if (($_SESSION ['cart'] [$id] ['num'] + $num) > $data_content['product_number']){
				die('ERR_'.$lang_module ['cart_set_err_num']);
			}
			$_SESSION ['cart'] [$id] ['num'] = $_SESSION ['cart'] [$id] ['num'] + $num;
		}
		...
		foreach ( $listid as $id ) {
			...
			if (! isset ( $_SESSION ['cart'] [$id] ))
				$_SESSION ['cart'] [$id] = array ('num' => $num, 'order' => 0,'price' => $data_content ['product_price']);
			else {
				$_SESSION ['cart'] [$id] ['num'] = $_SESSION ['cart'] [$id] ['num'] + $num;
			}
			$i ++;
