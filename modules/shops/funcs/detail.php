...
$data_content = $db->sql_fetchrow( $query, 2 );
	$sq = "SELECT sourceid, " . NV_LANG_DATA . "_title as title FROM `" . $db_config['prefix'] . "_" . $module_data . "_sources` WHERE sourceid = ".$data_content['source_id'];
	//die($sq);
	$r = $db->sql_query( $sq );
	$data_source = $db->sql_fetchrow( $r, 2 );
	$data_content['title_source'] = $data_source['title'];
$data_shop = array();
...
$s = "SELECT id, " . NV_LANG_DATA . "_title," . NV_LANG_DATA . "_alias, homeimgthumb,addtime, product_price,product_discounts, money_unit,showprice," . NV_LANG_DATA . "_hometext, homeimgfile  FROM `" . $db_config['prefix'] . "_" . $module_data . "_rows` WHERE id!= " . $id . " AND listcatid = " . $data_content['listcatid'] . " AND inhome=1 AND status=1 AND publtime < " . NV_CURRENTTIME . " AND (exptime=0 OR exptime>" . NV_CURRENTTIME . ") ORDER BY ID DESC LIMIT " . ( $pro_config['per_row'] * 2 );
$re = $db->sql_query( $s );
$link = NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=";
$data_others = array();
while ( list( $id, $title, $alias, $homeimgthumb, $addtime, $product_price, $product_discounts, $money_unit, $showprice,$hometext, $homeimgfile, $source_id ) = $db->sql_fetchrow( $re ) )
{
   $thumb = explode( "|", $homeimgthumb );
    ...
    $data_others[] = array( 
        "id" => $id, "title" => $title, "alias" => $alias, "hometext"=>$hometext, "homeimgthumb" => $thumb[0], "addtime" => $addtime, "product_price" => $product_price, "product_discounts" => $product_discounts, "money_unit" => $money_unit, "showprice" => $showprice, "link_pro" => $link . $global_array_cat[$data_content['listcatid']]['alias'] . "/" . $alias . "-" . $id, "link_order" => $link . "setcart&amp;id=" . $id, "homeimgfile"=>$homeimgfile, "title_source" => $data_source['title']
    );
}
...
if ( ! empty( $_SESSION[$module_data . '_proview'] ) )
{
    ...
    $s = "SELECT id, " . NV_LANG_DATA . "_title," . NV_LANG_DATA . "_alias, homeimgthumb,addtime, product_price,product_discounts, money_unit,showprice," . NV_LANG_DATA . "_hometext, homeimgfile  FROM `" . $db_config['prefix'] . "_" . $module_data . "_rows` WHERE id IN ( " . $arrtempid . ") AND inhome=1 AND status=1 AND publtime < " . NV_CURRENTTIME . " AND (exptime=0 OR exptime>" . NV_CURRENTTIME . ") ORDER BY ID DESC LIMIT " . ( $pro_config['per_row'] * 2 );
    $re = $db->sql_query( $s );
    $link = NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=";
    while ( list( $id, $title, $alias, $homeimgthumb, $addtime, $product_price, $product_discounts, $money_unit, $showprice,$hometext, $homeimgfile ) = $db->sql_fetchrow( $re ) )
    {
       $thumb = explode( "|", $homeimgthumb );
		$homeimgfile = NV_BASE_SITEURL . NV_UPLOADS_DIR . "/" . $module_name . "/".$homeimgfile;
        if ( ! empty( $thumb[0] ) && ! nv_is_url( $thumb[0] ) )
        ...
        $array_other_view[] = array( 
            "id" => $id, "title" => $title, "alias" => $alias, "hometext"=>$hometext, "homeimgthumb" => $thumb[0], "addtime" => $addtime, "product_price" => $product_price, "product_discounts" => $product_discounts, "money_unit" => $money_unit, "showprice" => $showprice, "link_pro" => $link . $global_array_cat[$data_content['listcatid']]['alias'] . "/" . $alias . "-" . $id, "link_order" => $link . "setcart&amp;id=" . $id, "homeimgfile"=>$homeimgfile
        );
    }
}
