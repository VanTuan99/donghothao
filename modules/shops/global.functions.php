...
if (!empty($pro_config)) {$temp = explode("x", $pro_config['image_size']);
...
function nv_comment_module($id, $page) {
	...
	while ($row = $db -> sql_fetchrow($comment)) {$row['post_email'] = ($module_config[$module_name]['emailcomm']) ? $row['post_email'] : "";
...
function nv_del_content_module($id) {
	...
	if ($id > 0) {
		if ($homeimgfile != "" or $homeimgthumb != "") {$homeimgfile .= "|" . $homeimgthumb;
			$homeimgfile_arr = explode("|", $homeimgfile);
			foreach ($homeimgfile_arr as $homeimgfile_i) {
				if (!empty($homeimgfile_i) and is_file(NV_UPLOADS_REAL_DIR . '/' . $module_name . '/' . $homeimgfile_i)) {@nv_deletefile(NV_UPLOADS_REAL_DIR . '/' . $module_name . '/' . $homeimgfile_i);
				}
			}
		}$number_no_del = 0;
		$array_catid = explode(",", $listcatid);
		if ($number_no_del == 0) {$query = "DELETE FROM `" . $db_config['prefix'] . "_" . $module_data . "_rows` WHERE `id`=" . $id;
			$db -> sql_query($query);
			if (!$db -> sql_affectedrows()) { {++$number_no_del;
				}$db -> sql_freeresult();
			}
		}
		if ($number_no_del == 0) {$db -> sql_query("DELETE FROM `" . $db_config['prefix'] . "_" . $module_data . "_comments` WHERE `id` = " . $id);
			$db -> sql_query("DELETE FROM `" . $db_config['prefix'] . "_" . $module_data . "_block` WHERE `id` = " . $id);
			$content_del = "OK_" . $id;
		} else {$content_del = "ERR_" . $lang_module['error_del_content'];
		}
	}
	return $content_del;
}
...
function nv_archive_content_module($id, $listcatid) {
	...
	$array_catid = explode(",", $listcatid);
	foreach ($array_catid as $catid_i) {$catid_i = intval($catid_i);
		if ($catid_i > 0) {$db -> sql_query("UPDATE `" . NV_PREFIXLANG . "_" . $module_data . "_" . $catid_i . "` SET `archive`='2' WHERE `id`=" . $id . "");
		}
...
function nv_products_page2($base_url, $num_items, $per_page, $start_item, $add_prevnext_text = true) {
...
function nv_file_table($table) {
	...
	while (list($field) = $db -> sql_fetchrow($result)) {$tmp = explode("_", $field);
		foreach ($lang_value as $lang_i) {
			if (!empty($tmp[0]) && !empty($tmp[1])) {
				if ($tmp[0] == $lang_i) {$arrfield[] = array($tmp[0], $tmp[1]);
					break;
				}
			}
		}
...
function nv_list_lang() {
	...
	$lang_value = array();
	while (list($lang_i) = $db -> sql_fetchrow($re)) {$lang_value[] = $lang_i;
	}
	return $lang_value;
}
...
function nv_number_format($number) {$temp = explode(".", $number);
	if (sizeof($temp) == 2) {$strlen = strlen($temp[1]);
