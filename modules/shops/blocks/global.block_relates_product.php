...
if ( ! function_exists( 'nv_relates_product' ) )
{
    ...
    function nv_relates_product ( $block_config )
    {
        ...
        $sql = "SELECT id,listcatid, " . NV_LANG_DATA . "_title," . NV_LANG_DATA . "_alias,addtime,homeimgthumb,product_price,product_discounts,money_unit,showprice,product_code FROM `" . $db_config['prefix'] . "_" . $mod_data . "_rows` ORDER BY ratingdetail DESC LIMIT 0,4";
        $query = $db->sql_query( $sql );
        $i = 1;
        while ( list( $id_i, $listcatid_i, $title_i, $alias_i, $addtime_i, $homeimgthumb_i, $product_price_i, $product_discounts_i, $money_unit_i, $showprice_i, $product_code_i ) = $db->sql_fetchrow( $query ) )
        {
