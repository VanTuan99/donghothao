...
if ($module_info['rss']) {$rss[] = array('title' => $module_info['custom_title'], 'src' => NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=rss");
}$arr_cat_title = array();
...
while (list($catid_i, $parentid_i, $lev_i, $title_i, $alias_i, $viewcat_i, $numsubcat_i, $subcatid_i, $numlinks_i, $del_cache_time_i, $description_i, $inhome_i, $keywords_i, $who_view_i, $groups_view_i) = $db -> sql_fetchrow($result)) {$link_i = NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=" . $alias_i . "";
	...
	if ($alias_cat_url == $alias_i) {$catid = $catid_i;
		$parentid = $parentid_i;
	}
	if ($module_info['rss']) {$rss[] = array('title' => $module_info['custom_title'] . ' - ' . $title_i, 'src' => NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=rss/" . $alias_i);
	}
...
$result = $db -> sql_query($sql);
while (list($groupid_i, $parentid_i, $lev_i, $title_i, $alias_i, $viewgroup_i, $numsubgroup_i, $subgroupid_i, $numlinks_i, $del_cache_time_i, $description_i, $inhome_i, $keywords_i, $who_view_i, $groups_view_i) = $db -> sql_fetchrow($result)) {$link_i = NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=group/" . $alias_i . "-" . $groupid_i;
...
if (!empty($array_op) and $op == "main") {
	if ($catid == 0) {$contents = $lang_module['nocatpage'] . $array_op[0];
		if (!empty($array_op[1])) {
			if (substr($array_op[1], 0, 5) == "page-") {$page = intval(substr($array_op[1], 5));
			}
		}
	} else {$op = "main";
		if ($count_op == 1 or substr($array_op[1], 0, 5) == "page-") {$op = "viewcat";
			if ($count_op > 1) {$page = intval(substr($array_op[1], 5));
			}
		} elseif ($count_op == 2) {$array_page = explode("-", $array_op[1]);
			$id = intval(end($array_page));
			$number = strlen($id) + 1;
			$alias_url = substr($array_op[1], 0, -$number);
			if ($id > 0 and $alias_url != "") {$op = "detail";
			}
		}
		$parentid = $catid;
		while ($parentid > 0) {$array_cat_i = $global_array_cat[$parentid];
...
function GetCatidInParent($catid) {
	...
	if (!empty($subcatid)) {
		foreach ($subcatid as $id) {
			if ($id > 0) {
				if ($global_array_cat[$id]['numsubcat'] == 0) {$array_cat[] = $id;
				} else {$array_cat_temp = GetCatidInParent($id);
					foreach ($array_cat_temp as $catid_i) {$array_cat[] = $catid_i;
					}
				}
			}
		}
...
function GetDataInGroup($result, $groupid) {
	...
	$data_content = array();
	$data = array();
	while (list($id, $listcatid, $publtime, $title, $alias, $hometext, $address, $homeimgalt, $homeimgthumb, $product_price, $product_discounts, $money_unit, $showprice, $homeimgfile) = $db -> sql_fetchrow($result)) {$thumb = explode( "|", $homeimgthumb );
		$homeimgfile = NV_BASE_SITEURL . NV_UPLOADS_DIR . "/" . $module_name . "/".$homeimgfile;
        if ( ! empty( $thumb[0] ) && ! nv_is_url( $thumb[0] ) )
...
function CurrencyConversionToNumber($price, $currency_curent, $currency_convert) {
	...
	if ($currency_curent != $currency_convert) {
		if (isset($money_config[$currency_curent]['exchange'])) {$value = doubleval($money_config[$currency_curent]['exchange']);
			$price_convert = $price_convert * $value;
		} else {
			return false;
		}
		if ($currency_convert != $pro_config['money_unit']) {
			if (isset($money_config[$currency_convert]['exchange'])) {$value = doubleval($money_config[$currency_convert]['exchange']);
				$price_convert = $price_convert / $value;
			} else {
				return false;
			}
...
function SetSessionProView($id, $title, $alias, $addtime, $link, $homeimgthumb) {
	...
	if (!isset($_SESSION[$module_data . '_proview'][$id])) {$_SESSION[$module_data . '_proview'][$id] = array('title' => $title, 'alias' => $alias, 'addtime' => $addtime, 'link' => $link, 'homeimgthumb' => $homeimgthumb);
	}
}
