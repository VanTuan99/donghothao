...
if ( ! nv_function_exists( 'nv_menu_theme_dienhoa' ) )
{

    function nv_menu_theme_dienhoa ( $block_config )
    {
        global $home, $db, $db_config, $global_config, $site_mods, $module_info, $module_name, $module_file, $module_data, $lang_global, $catid;
        
        if ( file_exists( NV_ROOTDIR . "/themes/" . $global_config['site_theme'] . "/modules/menu/menu_theme_dienhoa.tpl" ) )
        {
        ...
        $xtpl = new XTemplate( "menu_theme_dienhoa.tpl", NV_ROOTDIR . "/themes/" . $block_theme . "/modules/menu" );
    ...
    foreach ( $site_mods2 as $modname => $modvalues )
    {
        if ( ! empty( $modvalues['in_menu'] ) )
        {
            ...
            $aryay_menu = array( 
                "module_file" => $module_file, "title" => $modvalues['custom_title'], "class" => $modname, "current" => $module_current, "link" => $link 
            );
...
{
    $content = nv_menu_theme_dienhoa( $block_config );
}
