...
if ($global_config['allowuserlogin'] and $module_name != "users") {
	if (file_exists(NV_ROOTDIR . "/themes/" . $global_config['module_theme'] . "/modules/users/block.login.tpl")) {$block_theme = $global_config['module_theme'];
	} elseif (file_exists(NV_ROOTDIR . "/themes/" . $global_config['site_theme'] . "/modules/users/block.login.tpl")) {$block_theme = $global_config['site_theme'];
	} else {$block_theme = "default";
	}
	$xtpl = new XTemplate("block.login.tpl", NV_ROOTDIR . "/themes/" . $block_theme . "/modules/users");
	if (defined('NV_IS_USER')) {$avata = "";
		...
		if (!defined('NV_IS_ADMIN')) {$xtpl -> assign('LOGOUT_ADMIN', "" . NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=users&amp;" . NV_OP_VARIABLE . "=logout");
		...
		if (!empty($groups_list) && $global_config['allowuserpublic'] == 1) {$in_group = "<a title='" . $lang_global['in_groups'] . "' href='" . NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=users&amp;" . NV_OP_VARIABLE . "=regroups'>" . $lang_global['in_groups'] . "</a>";
			$xtpl -> assign('in_group', $in_group);
		}$xtpl -> parse('signed');
		$content = $xtpl -> text('signed');
	} else {$xtpl -> assign('REDIRECT', nv_base64_encode($client_info['selfurl']));
		...
		$xtpl -> assign('LANG', $lang_global);
		if (in_array($global_config['gfx_chk'], array(2, 4, 5, 7))) {$xtpl -> assign('N_CAPTCHA', $lang_global['securitycode']);
		...
		if (defined('NV_OPENID_ALLOWED')) {$xtpl -> assign('OPENID_IMG_SRC', NV_BASE_SITEURL . "themes/" . $block_theme . "/images/users/openid_small.gif");
			$xtpl -> assign('OPENID_IMG_WIDTH', 24);
			$xtpl -> assign('OPENID_IMG_HEIGHT', 24);
			$assigns = array();
			foreach ($openid_servers as $server => $value) {$assigns['href'] = NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=users&amp;" . NV_OP_VARIABLE . "=login&amp;server=" . $server . "&amp;nv_redirect=" . nv_base64_encode($client_info['selfurl']);
