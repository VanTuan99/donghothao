...
if (!defined('NV_MAINFILE'))
	die('Stop!!!');
if (!nv_function_exists('nv_news_block_tophits_2')) {
	function nv_block_config_tophits_2_blocks($module, $data_block, $lang_block) {$html = "";
		$html .= "<tr>";
	...
	function nv_block_config_tophits_2_blocks_submit($module, $lang_block) {
		global $nv_Request;
		$return = array();
		$return['error'] = array();
		$return['config'] = array();
	...
	function nv_news_block_tophits_2($block_config, $mod_data) {
		...
		if (file_exists(NV_ROOTDIR . "/themes/" . $module_info['template'] . "/modules/news/block_news_2.tpl")) {$block_theme = $module_info['template'];
		} else {$block_theme = "default";
		}$xtpl = new XTemplate("block_news_2.tpl", NV_ROOTDIR . "/themes/" . $block_theme . "/modules/news");
		$a = 1;
		foreach ($array_block_news as $array_news) {$xtpl -> assign('blocknews', $array_news);
			if (!empty($array_news['imgurl'])) {$xtpl -> parse('main.newloop.imgblock');
...
if (defined('NV_SYSTEM')) {
	...
	if (isset($site_mods[$module])) {$mod_data = $site_mods[$module]['module_data'];
		...
		}$content = nv_news_block_tophits_2($block_config, $mod_data);
	}
}
?>