...
if (!nv_function_exists('nv_block_counter')) 
{
	function nv_block_counter() 
	{
		...
		while (list($c_type, $c_val, $c_count) = $db -> sql_fetchrow($query)) 
		{
			if ($c_type == 'day' and $c_val == date('d', NV_CURRENTTIME))
			 {$xtpl -> assign('COUNT_DAY', $c_count);}
			  elseif ($c_type == 'month' and $c_val == date('M', NV_CURRENTTIME)) {$xtpl -> assign('COUNT_MONTH', $c_count);} 
			  elseif ($c_type == 'total' and $c_val == 'hits') {$xtpl -> assign('COUNT_ALL', $c_count);}
		}
		...
		while ($row = $db -> sql_fetchrow($query)) {++$count_online;
			if ($row['uid'] != 0) {++$users;
			} else {
		...
		if ($users) {$xtpl -> assign('COUNT_USERS', $users);
			$xtpl -> parse('main.users');
		}
		if ($bots) {$xtpl -> assign('COUNT_BOTS', $bots);
			$xtpl -> parse('main.bots');
		}
		if ($guests and $guests != $count_online) {$xtpl -> assign('COUNT_GUESTS', $guests);
...
if (defined('NV_SYSTEM')) 
{
	...
	if ($global_config['online_upd']) {$content = nv_block_counter();}
}
?>